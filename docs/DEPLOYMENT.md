...
| `011-tool-trays` | Tool tray designer (pockets of different shapes and depths for a printer tool kit, on the shared Systainer outlines) | `20260910T115600422Z-62e954b580deee76` (634,880 bytes), taken automatically before the migration ran. The first row under the policy below to carry a real id rather than a choice. It took three attempts to get here and the first is worth knowing about: the snapshot could not be written at all, because the artifact-store guard re-established a staged file's identity from its inode and timestamps, which Azure Files cannot honour -- so the release refused to change the schema and rolled back, exactly as it should have. Identity now comes from the bytes (`4d73382`). `tool_tray_designs` is a new table no prior release reads and nothing else references. |
